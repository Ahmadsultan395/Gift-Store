  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 z-10 flex scale-95 items-center justify-center bg-primary-600/20 opacity-0 backdrop-blur-[1px] transition-all duration-300 group-hover:scale-100 group-hover:opacity-100">
        <span className="rounded-full bg-primary-600 px-4 py-2 text-xs font-semibold text-white shadow-lg">
      ...
      {pages.map((p, i) =>
        p === "..." ? (
          ...
          <button
            key={p}
            onClick={() => onChange(p)}
            className={`flex h-9 min-w-9 items-center justify-center rounded-lg px-2 text-sm font-semibold transition-all duration-200 ${
              p === page
                ? "scale-105 bg-primary-600 text-white shadow-md shadow-primary-200"
...
function ReviewModal({ onClose, onSubmitted }) {
  ...
  return (
    ...
    >
      <div
        className={`max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl transition-all duration-300 ease-out ${
          visible ? "scale-100 opacity-100" : "scale-95 opacity-0"
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        <TestimonialForm onSuccess={onSubmitted} onClose={onClose} />
      </div>
    </div>
  );
}
    ...
    <section id="testimonials-section" className="py-4">
      ...
      <div className="mt-8 flex justify-center">
        <button
          ...
          className="flex items-center gap-2 rounded-xl bg-primary-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary-700 hover:shadow-lg hover:shadow-primary-200"
        >
          <PenSquare size={16} />
          Write a Review
        </button>
