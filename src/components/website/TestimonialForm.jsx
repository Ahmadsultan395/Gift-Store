...
export default function TestimonialForm({ existing = null, onSuccess, onClose }) {
    ...
    return (
      <div className="relative flex flex-col items-center gap-4 rounded-2xl bg-white p-8 text-center">
        ...
        <p className="font-semibold text-slate-800 text-base leading-snug">
          Thank you! Your testimonial has been submitted for review and will appear after admin approval.
        </p>
        <button onClick={onClose}
  ...
  return (
    <form onSubmit={handleSubmit} className="relative space-y-4 rounded-2xl bg-white p-1">
      {onClose && (
      ...
      {isEditing && existing?.status === "pending" && (
        <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-700">
          Your testimonial is currently pending approval. Editing will re-submit it for review.
        </div>
      )}
      {isEditing && existing?.status === "rejected" && (
        <div className="rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">
          Your testimonial was not approved. You can edit and re-submit it.
          {existing.adminNote && <span className="block mt-1 font-medium">Note: {existing.adminNote}</span>}
        </div>
      )}
      ...
      {/* Photo optional */}
      <div>
        <label className="mb-1 block text-sm font-medium text-slate-600">Photo (optional)</label>
        <input type="file" accept="image/*" onChange={handlePhotoChange}/>
        {photoPreview && (
          <img src={photoPreview} alt="Preview" className="mt-2 h-16 w-16 rounded-full object-cover border-2 border-primary-200"/>
        )}
      </div>
      ...
      <button type="submit" disabled={submitting}
        ...
        {submitting ? "Submitting..." : isEditing ? "Update Testimonial" : "Submit Testimonial"}
      </button>
    </form>
  );
}
