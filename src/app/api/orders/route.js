...
export async function POST(request) {
  try {
    ...
    const { name, phone, address, city, notes, paymentMethod, items, couponCode } = body;

    if (!name || !phone || !address || !city) return fail("Shipping info is required");
    ...
    for (const item of items) {
      const product = await Product.findById(item.product).select("name sellingPrice stock images");
      if (!product) return fail(`Product not found: ${item.product}`);
      if (product.stock < item.quantity) return fail(`"${product.name}" only has ${product.stock} in stock`);
      const total = product.sellingPrice * item.quantity;
      subTotal += total;
      enrichedItems.push({
        product: product._id, name: product.name,
        image: product.images?.[0]?.url || "",
        quantity: item.quantity, price: product.sellingPrice, total,
      });
    }
    ...
    // Update coupon usage
    if (usedCoupon) await Coupon.findByIdAndUpdate(usedCoupon._id, { $inc: { usedCount: 1 } });

    // Update customer stats
    if (custId) await Customer.findByIdAndUpdate(custId, { $inc: { totalOrders: 1 } });

    // New order notification for admin
    await Notification.create({ type: "new_order", title: "New Order Received", message: `Order ${orderNumber} placed by ${name}`, relatedId: order._id });
