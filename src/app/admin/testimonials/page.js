...
export default function AdminTestimonialsPage() {
  ...
  async function fetchData() {
    ...
    try {
      const res = await fetch(`/api/admin/testimonials?${p}`, {
        cache: "no-store",
      });
  ...
  return (
    <div>
      <PageHeader
        title="Testimonials"
        subtitle="Manage customer testimonials — approve, reject or delete."
      />
            ...
            {testimonials.map((t) => (
              <div
                ...
                className={`flex flex-col justify-between rounded-xl border p-4 shadow-sm transition-all ${
                  t.status === "approved"
                    ? "border-primary-200 bg-green-50/30"
                    : t.status === "rejected"
      ...
      <ConfirmDialog
        ...
        message={`Delete "${del?.name}"'s testimonial? This cannot be undone.`}
      />
